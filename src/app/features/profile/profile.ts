        ...
        <form class="panel-body" [formGroup]="form" (ngSubmit)="save()">
          ...
          <div class="form-field">
            ...
            <input
              agroInput
              id="profile-role"
              [value]="auth.user()?.role || 'No informado'"
              readonly
            />
          </div>
          <div class="demo-note">
...
export class Profile {
  ...
  readonly avatar = signal(this.auth.user()?.avatar ?? null);
  ...
  reset(): void {
    this.form.reset({ name: this.auth.user()?.name ?? '', email: this.auth.user()?.email ?? '' });
    this.avatar.set(this.auth.user()?.avatar ?? null);
  }
